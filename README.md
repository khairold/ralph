...
```
/project:prd [your feature description]
```
...
```
/project:ralph tasks/prd-[feature-name].md
```
...
Claude Code is invoked with the `--chrome` flag, enabling browser automation for frontend story verification. This replaces manual UI testing with automated visual verification.
...
Frontend stories must include "Verify in browser using Claude Code's browser tools" in acceptance criteria. Ralph uses the `--chrome` flag to enable browser automation, allowing it to navigate to pages, interact with the UI, and confirm changes work.
